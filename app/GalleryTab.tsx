...
export default function GalleryTab({ section, title, selected }: { section: SectionType, title: string, selected: boolean}) {
    return (
        <Link className="w-full" href={section == "paintings" ? "/" : "/?section=sculptures"}>
            <button className={`cursor-pointer w-full flex justify-center items-center gap-2 border-b-2 ${selected ? "border-gray-700 text-gray-800" : "border-gray-200 text-gray-400"} h-[60px] smoothe ${openSans.medium}`}>
